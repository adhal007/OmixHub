...
class OutlierStatMethod:
    ...
    def get_all_meds(self, data: pd.DataFrame):
        return [
            self.get_median(data.iloc[:, i].to_numpy()) for i in range(data.shape[1])
        ]

    def get_all_mads(self, data: pd.DataFrame):
        # Convert DataFrame to NumPy array once and iterate over its columns
        data_np = data.to_numpy()
        return [self.get_mad(data_np[:, i]) for i in range(data_np.shape[1])]
    ...
    def apply_mad_norm(self, data: pd.DataFrame):
        ...
        data_norm = [
            (data.iloc[:, i].to_numpy() - all_meds[i]) / all_mads[i]
            for i in range(data.shape[1])
        ]
    ...
    def get_mad(self, array):
        # Assuming you have a predefined get_mad function since it's not provided
